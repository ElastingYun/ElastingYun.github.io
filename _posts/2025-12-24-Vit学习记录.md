...
```mermaid
flowchart TD
    A[Input Image<br/>(H × W × 3)] --> B[Patchify<br/>(Split into P×P patches)]
    B --> C[Flatten<br/>(P×P×3 → Vector)]
    C --> D[Linear Projection<br/>(Patch Embedding)]
    D --> E[Patch Tokens<br/>(N × d_model)]
    E --> F[Add CLS Token]
    F --> G[Add Position Embedding]
    G --> H[Transformer Encoder<br/>(MSA + FFN)]
    H --> I[Output Tokens]
    I --> J[CLS Token → Classification Head]
    I --> K[Patch Tokens → Dense Prediction Tasks]
```
###
